SUBROUTINE RGNoptim(x, f) bind(c, name="rgnoptim_")
...
!******************************************************************
   ...
   REAL(rk) :: x0(p), xLo(p), xHi(p)
   REAL(rk), intent(out) :: x(p), f
...
!   message=""                                           ! Initialize message
   ...
   CALL rgn (objFunc=objFunc, p=2_ik, n=2_ik, x0=x0, xLo=xlo, xHi=Xhi, cnv=cnv, &
      x=x, info=info, error=error, message=message)
   IF(error /= 0)then
     WRITE(*,*) message
     READ(*,*)
   END IF
